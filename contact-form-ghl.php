...
function cfg_ghl_ensure_fields( $api_key, $location_id, $s ) {
    ...
    $router_folder = $make_folder( 'Router' );
...
function cfg_ajax_save_folder_ids() {
    ...
    $map = [];
    foreach ( [ 'Contact Form', 'Router', 'Invisalign Form', 'Implants Form', 'UTM Forms' ] as $name ) {
...
// ═══════════════════════════════════════════════════════════════
//  Auto-provision the "Patient Type" custom field in GHL AND move
//  it into the "Contact Form" folder. Runs once per site the first
//  time an admin loads any admin page after upgrading. Skips
//  silently if the API key or location ID isn't configured yet —
//  will retry on the next admin visit.
// ═══════════════════════════════════════════════════════════════
add_action( 'admin_init', 'cfg_auto_provision_patient_type_field' );
function cfg_auto_provision_patient_type_field() {
    if ( ! current_user_can( 'manage_options' ) ) return;
    // v2: bumped from v1 to re-run and fix folder placement for sites that already
    // got the field but had it dumped outside the Contact Form folder.
    if ( get_option( 'cfg_patient_type_field_v2' ) === '1' ) return;
    ...
    // ── 1) Resolve (or create) the "Contact Form" folder ID FIRST ──
    $folder_id = null;
    $fr = wp_remote_get( "{$base}/locations/{$location_id}/customFieldsFolders", [ 'headers' => $headers, 'timeout' => 15 ] );
    if ( ! is_wp_error( $fr ) && wp_remote_retrieve_response_code( $fr ) < 300 ) {
        $fb = json_decode( wp_remote_retrieve_body( $fr ), true );
        foreach ( ( $fb['folders'] ?? [] ) as $folder ) {
            if ( strtolower( trim( $folder['name'] ?? '' ) ) === 'contact form' ) { $folder_id = $folder['id']; break; }
        }
    }
    if ( ! $folder_id ) {
        $cr = wp_remote_post( "{$base}/locations/{$location_id}/customFieldsFolders", [
        ...
        if ( ! is_wp_error( $cr ) && wp_remote_retrieve_response_code( $cr ) < 300 ) {
            $cb        = json_decode( wp_remote_retrieve_body( $cr ), true );
            $folder_id = $cb['folder']['id'] ?? $cb['id'] ?? null;
        }
    }

    // ── 2) Find (or create) the patient_type field ──
    $r = wp_remote_get( "{$base}/locations/{$location_id}/customFields", [ 'headers' => $headers, 'timeout' => 15 ] );
    if ( is_wp_error( $r ) ) return; // network hiccup — retry next visit
    $code = wp_remote_retrieve_response_code( $r );
    if ( $code === 401 || $code === 403 ) { update_option( 'cfg_patient_type_field_v2', '1' ); return; }
    ...
    $field_id = null;
    if ( $existing ) {
        $field_id = $existing['id'] ?? null;
        // Keep options in sync (preserve any admin-added extras)
        $cur_type = $existing['dataType'] ?? 'TEXT';
        $cur_opts = $existing['options']  ?? [];
        if ( $cur_type === 'SINGLE_OPTIONS' ) {
            $missing = array_diff( $desired_options, $cur_opts );
            if ( ! empty( $missing ) ) {
        ...
        // Create fresh (include parentId — some tenants honor it on create)
        $payload = [
            'name'     => 'Patient Type',
        ...
        if ( $ccode >= 200 && $ccode < 300 ) {
            ...
            return; // real failure — retry next admin visit
        }
    }

    // ── 3) FORCE-MOVE the field into the Contact Form folder ──
    // GHL's create endpoint often ignores parentId. Always follow up with a PUT so
    // the field is guaranteed to end up in the right folder. This is idempotent —
    // safe to run every time; PUT with the same parentId is a no-op.
    if ( $field_id && $folder_id ) {
        wp_remote_request( "{$base}/locations/{$location_id}/customFields/{$field_id}", [
            'method'  => 'PUT',
            'headers' => $headers,
            'body'    => wp_json_encode( [ 'parentId' => $folder_id ] ),
    ...
    update_option( 'cfg_patient_type_field_v2', '1' );
    delete_option( 'cfg_utm_key_map_' . md5( $location_id ) );
}
...
&lt;/script&gt;</pre>
    ...
    <div id="cfg-ghl_fields" class="cfg-panel">
        ...
        <div class="cfg-panel-body">
            ...
            $folder_names = [ 'Contact Form', 'Router', 'Invisalign Form', 'Implants Form', 'UTM Forms' ];
            ?>
                ...
                <div style="display:flex;align-items:flex-start;justify-content:space-between;flex-wrap:wrap;gap:10px;margin-bottom:14px;">
                    <div>
                        ...
                        <div style="margin-top:8px;font-size:11px;color:#64748b;line-height:1.9;">
                            ...
                            <strong style="color:#2563eb;">Step 1.</strong> In GHL → Settings → Custom Fields → create 5 folders named exactly:&nbsp;
                            <code style="background:#e2e8f0;padding:1px 5px;border-radius:3px;">Contact Form</code>&nbsp;
                            <code style="background:#e2e8f0;padding:1px 5px;border-radius:3px;">Router</code>&nbsp;
                            ...
                            <strong style="color:#2563eb;">Step 2.</strong> Click <strong>+ Create Checker Fields</strong> — 5 temporary marker fields will appear in GHL under Additional Info<br>
            ...
            (function(){
                ...
                var FOLDER_KEYS = ['contact_form','router','invisalign_form','implants_form','utm_forms'];
                var FOLDER_NAMES = {'contact_form':'Contact Form','router':'Router','invisalign_form':'Invisalign Form','implants_form':'Implants Form','utm_forms':'UTM Forms'};
                ...
                document.getElementById('cfg-detect-folders-btn').addEventListener('click', function(){
                    ...
                    .then(function(res){
                        ...
                        if (nameMatches > 0) {
                            ...
                            fst.textContent = '✓ Matched ' + nameMatches + ' of 5 folders by name — saved automatically.';
                            fst.style.color = nameMatches === 5 ? '#16a34a' : '#d97706';
                            return;
                        }
                        ...
                        var checkerKeys = {
                            'contact_form':'cfg_checker_contact_form','router':'cfg_checker_router',
                            'invisalign_form':'cfg_checker_invisalign_form',
                        ...
                        .then(function(r){ return r.json(); })
                        .then(function(sr){
                            var msg = (sr.success ? '✓ Auto-filled & saved ' : '⚠ Filled but save failed — ') + filled + ' of 5 folder IDs.';
                            if (unmatched.length) msg += ' ' + unmatched.length + ' unrecognised: ' + unmatched.join(', ');
                            if (filled < 5) msg += ' — ' + (5 - filled) + ' still missing, paste manually then Save.';
                            fst.textContent = msg;
                            fst.style.color = sr.success && filled === 5 ? '#16a34a' : '#d97706';
                        })
...
.<?= $uid ?>-scard-hd{padding:0.8rem 1.1rem;border-bottom:1px solid <?= esc_attr($s['border_color']) ?>;display:flex;align-items:center;gap:0.5rem;}
.<?= $uid ?>-scard-hd span{font-size:0.7rem;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;color:<?= esc_attr($s['muted_color']) ?>;}
.<?= $uid ?>-scard-hd i{font-size:0.8rem;color:<?= esc_attr($s['muted_color']) ?>;}
.<?= $uid ?>-scard-bd{padding:0.1rem 1.1rem 0.35rem;}
.<?= $uid ?>-chip{display:flex;align-items:baseline;gap:0.4rem;padding:0.55rem 0;border-bottom:1px solid <?= esc_attr($s['border_color']) ?>;font-size:0.82rem;line-height:1.4;}
...
<div id="<?= $uid ?>-wrap">
  ...
  <div id="<?= $uid ?>-grid">
    <div>
      <div id="<?= $uid ?>-outer">
        ...
        <?php foreach ( $steps as $i => $step ):
            ...
            $qlbl = esc_attr( $step['question'] ?? $step['title'] ?? '' );
            echo '<div class="' . $uid . '-step" data-index="' . $i . '" data-type="' . esc_attr( $type ) . '" data-key="' . $fk . '" data-label="' . $qlbl . '">';

            // ── INTRO ──────────────────────────────────────────────
            if ( $type === 'intro' ) {
                $bullets = array_filter( array_map( 'trim', explode( "\n", $step['bullets'] ?? '' ) ) );
                echo '<div style="display:inline-flex;align-items:center;gap:0.5rem;background:rgba(0,0,0,0.055);border-radius:99px;padding:0.3rem 0.9rem;margin-bottom:1.5rem;">';
                echo '<span style="width:6px;height:6px;border-radius:50%;background:' . $accent . ';display:inline-block;"></span>';
                echo '<span style="font-size:0.75rem;font-weight:600;color:' . $tc . ';letter-spacing:0.04em;">Invisalign Assessment</span>';
                echo '</div>';
                echo '<h1 style="font-size:clamp(2rem,5vw,3rem);font-weight:800;color:' . $tc . ';margin:0 0 1rem;line-height:1.15;letter-spacing:-0.03em;">' . esc_html( $step['title'] ?? '' ) . '</h1>';
                if ( ! empty( $step['subtitle'] ) ) {
                    echo '<p style="font-size:1rem;color:' . esc_attr($s['muted_color']) . ';margin:0 0 2rem;line-height:1.7;max-width:460px;">' . esc_html( $step['subtitle'] ) . '</p>';
                }
                if ( $bullets ) {
                    echo '<div style="margin-bottom:2rem;">';
                    foreach ( $bullets as $b ) {
                        echo '<div style="display:flex;align-items:flex-start;gap:0.75rem;padding:0.55rem 0;border-bottom:1px solid ' . esc_attr($s['border_color']) . ';">';
                        echo '<i class="fa-solid fa-check" style="color:' . $accent . ';font-size:0.8rem;margin-top:0.25rem;flex-shrink:0;"></i>';
                        echo '<span style="color:' . $tc . ';font-size:0.9rem;line-height:1.55;">' . esc_html( $b ) . '</span>';
                        echo '</div>';
                    }
                    echo '</div>';
                }
            ...
            elseif ( $type === 'yesno' ) {
                ...
                echo '<h2 style="font-size:clamp(1.4rem,3.5vw,2rem);font-weight:700;color:' . esc_attr($s['text_color']) . ';margin:0 0 ' . ( $has_hint ? '0.5rem' : '1.5rem' ) . ';line-height:1.25;letter-spacing:-0.02em;">' . esc_html( $step['question'] ?? '' ) . '</h2>';
                if ( $has_hint ) echo '<p style="color:' . esc_attr($s['muted_color']) . ';font-size:0.875rem;margin:0 0 1.5rem;line-height:1.6;">' . esc_html( $step['hint'] ) . '</p>';
                echo '<div class="' . $uid . '-choices">';
                echo '<div class="' . $uid . '-card" data-value="Yes" onclick="' . $uid . 'yn(this)">Yes</div>';
                echo '<div class="' . $uid . '-card" data-value="No" onclick="' . $uid . 'yn(this)">No</div>';
                echo '</div>';
            ...
            elseif ( $type === 'contact' ) {
                if ( $i > 0 ) echo '<div class="' . $uid . '-backrow"><button class="' . $uid . '-ghost" onclick="' . $uid . 'go(' . ( $i - 1 ) . ')"><i class="fa-solid fa-arrow-left" style="font-size:0.8em;"></i> Go back</button></div>';
                echo '<h2 style="font-size:clamp(1.4rem,3.5vw,2rem);font-weight:700;color:' . esc_attr($s['text_color']) . ';margin:0 0 0.4rem;line-height:1.25;letter-spacing:-0.02em;">' . esc_html( $step['title'] ?? 'Get Your Assessment' ) . '</h2>';
                if ( ! empty( $step['subtitle'] ) ) echo '<p style="color:' . esc_attr($s['muted_color']) . ';margin:0 0 1.75rem;font-size:0.9rem;line-height:1.65;">' . esc_html( $step['subtitle'] ) . '</p>';
    ...
    <!-- Sidebar -->
    <div id="<?= $uid ?>-sidebar">
      <div id="<?= $uid ?>-sb-intro" class="<?= $uid ?>-scard">
        <div class="<?= $uid ?>-scard-hd">
          <i class="fa-regular fa-clipboard"></i>
          <span>Your Assessment</span>
        </div>
        <div class="<?= $uid ?>-scard-bd">
          <?php
          $alg_preview = [];
          foreach ( $steps as $pst ) {
              if ( in_array( $pst['type'] ?? '', ['yesno','image'] ) && ! empty( $pst['question'] ) ) {
                  $alg_preview[] = $pst['question'];
              }
          }
          foreach ( array_slice( $alg_preview, 0, 5 ) as $pv ):
          ?>
          <div style="display:flex;align-items:flex-start;gap:0.6rem;padding:0.55rem 0;border-bottom:1px solid <?= esc_attr($s['border_color']) ?>;">
            <i class="fa-regular fa-circle-check" style="color:<?= esc_attr($s['muted_color']) ?>;font-size:0.8rem;margin-top:0.15rem;flex-shrink:0;"></i>
            <span style="font-size:0.82rem;color:<?= $tc ?>;line-height:1.45;"><?= esc_html($pv) ?></span>
          </div>
          <?php endforeach; ?>
          <div style="padding:0.65rem 0 0.35rem;display:flex;align-items:center;gap:0.4rem;font-size:0.73rem;color:<?= esc_attr($s['muted_color']) ?>;">
            <i class="fa-solid fa-circle" style="font-size:4px;"></i>
            Assessment accuracy improves with each step
          </div>
        </div>
      </div>
      <div id="<?= $uid ?>-sb-profile" class="<?= $uid ?>-scard" style="display:none;margin-top:1.25rem;">
        <div class="<?= $uid ?>-scard-hd">
          <i class="fa-regular fa-user"></i>
          <span>Your Profile</span>
        </div>
        <div class="<?= $uid ?>-scard-bd" id="<?= $uid ?>-chips">
          <div style="padding:0.75rem 0;font-size:0.82rem;color:<?= esc_attr($s['muted_color']) ?>;font-style:italic;">Your answers will appear here.</div>
        </div>
      </div>
    </div><!-- /sidebar -->
...
(function(){
    ...
    steps.forEach(function(s){ var k=s.dataset.key; var l=s.dataset.label; if(k&&l) labels[k]=l; });
    ...
    function updateSidebar(){
        var intro=document.getElementById(uid+'-sb-intro');
        var profile=document.getElementById(uid+'-sb-profile');
        if(!intro||!profile) return;
        if(cur===0){ intro.style.display='block'; profile.style.display='none'; }
        else { intro.style.display='none'; profile.style.display='block'; renderChips(); }
    }
